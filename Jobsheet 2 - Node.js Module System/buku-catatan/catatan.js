...
const bacaCatatan = function (judul) {
    const catatan = muatCatatan()
    const catatanDitemukan = catatan.filter(function (note) {
            return note.judul === judul
        })

    if (catatanDitemukan.length >= 1) {
        console.log(chalk.green(catatanDitemukan[0].isi))
    } else {
        console.log('Catatan tidak ditemukan/tersedia')
    }
}
...
const listCatatan = function () {
    ...
    for(const note of catatan) {
        console.log(`Judul: ${note.judul}, Isi: ${note.isi}`)
    }
}
